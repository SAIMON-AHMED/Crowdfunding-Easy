...
contract CrowdFundEasy {
    ...
    function createCampaign(uint256 _goal, uint256 _duration) external {
        ...
        campaignLists.push(Campaign(msg.sender, _goal, 0, (_duration + block.timestamp)));
    }
    ...
    function contribute(uint256 _id, uint256 _amount) external payable {
        ...
        require(campaign.duration > block.timestamp && campaign.creator != msg.sender);
    ...
    function cancelContribution(uint256 _id) external {
        Campaign storage campaign = campaignLists[_id -1];
        require(campaign.duration > block.timestamp && contributorRecords[_id -1][msg.sender] > 0);
        campaign.fundRaise -= contributorRecords[_id -1][msg.sender];
    ...
    function withdrawFunds(uint256 _id) external {
        Campaign storage campaign = campaignLists[_id -1];
        require(campaign.creator == msg.sender, "Only campaign creator can call this function");
        require(campaign.duration <= block.timestamp && token.getTokenPriceInUSD() * campaign.fundRaise >= campaign.goal);
        token.transfer(campaign.creator, campaign.fundRaise);
    ...
    function refund(uint256 _id) external {
        require(contributorRecords[_id -1][msg.sender] > 0, "You are not a contributor");
    ...
    public view returns (uint256) {
        return contributorRecords[_id -1][_contributor] * token.getTokenPriceInUSD();
    }
    ...
    function getCampaign(uint256 _id)
        ...
        returns (uint256 remainingTime, uint256 goal, uint256 totalFunds) {
            return (campaignLists[_id -1].duration - block.timestamp, campaignLists[_id -1].goal, campaignLists[_id -1].fundRaise * token.getTokenPriceInUSD());
        }
}
